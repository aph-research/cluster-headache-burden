...
| Lever | Default | Evidence |
|---|---|---|
| `preventive_access_fraction` | 0.23 | **[derived]** share of CH patients on an effective preventive, built with the SAME bottom-up HIC/MIC/LIC framework as `treatment_access_fraction` (pop weights developed 15% / intermediate 25% / developing 60%). Preventives (verapamil, corticosteroids) are cheap generics, so LMIC access exceeds that for O₂/triptans — but BOTH channels are gated by the same ~10.4-yr diagnostic delay, which caps how far preventive can pull ahead. Scenarios: conservative HIC .50/MIC .15/LIC .05 → **0.14**; moderate .55/.25/.10 → **0.21**; optimistic .55/.35/.18 → **0.28**. Central **0.23** = moderate, nudged up slightly for verapamil/prednisone leaking to GP-managed / partially-diagnosed patients. (Earlier default was a looser 0.30 assumption, which the bottom-up shows sits at the optimistic edge.) Sensitivity 0.15–0.55. **NB:** drawn INDEPENDENTLY of abortive access here, though in reality the two are positively correlated through diagnosis — a known simplification. |
